...
import type { AppConfig, ReviewLogger, ReviewOptions, ReviewReport, ReviewIssue } from "./reviewTypes.js";
...
async function runReviewSimple(
  ...
  logger.info(`分为 ${batches.length} 个批次（每批最多 ${batchSize} 个文件），并发: ${concurrency}`);
  ...
  return { report: merged, totalFiles: files.length, batchCount: batches.length, mode, strategy: "simple" };
}
...
function mergeReports(reports: ReviewReport[]): ReviewReport {
  if (reports.length === 0) {
    return { summary: "无评审结果", issues: [], score: { style: 0, logic: 0, robustness: 0, overall: 0 } };
  }
...
function smartMergeReports(quickReports: ReviewReport[], deepReports: ReviewReport[]): ReviewReport {
