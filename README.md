...
- **Git 仓库管理** — 增删改查仓库信息（地址、分支、访问令牌等），支持通过目录浏览器选择本地仓库并自动识别仓库信息；远程仓库创建后自动异步下载到默认目录 `data/repos`
- **LLM 配置管理** — 管理大语言模型配置（供应商、API Key、模型参数等），支持在线获取可用模型列表，支持 API 连通性测试（页面自动检测状态并显示绿/红点指示）
- **文件筛选模式** — 按项目类型预设文件筛选规则（包含扩展名 + 排除目录），内置 Unity 项目和 Node.js 项目两种预设，支持自定义新增；评审计划和任务创建时可选择筛选模式
- **评审计划管理** — 配置评审计划（名称、仓库、LLM、目标分支、文件筛选模式、触发时机），支持按间隔/每天定时自动触发，支持手动立即触发
- **评审任务管理** — 创建评审任务，关联仓库与 LLM 配置；支持任务进展可视化（阶段步骤条、批次进度条、Agent 执行对比条、Token/费用统计），运行中任务自动轮询刷新；评审计划触发时自动生成
...
| POST | /api/git-repos | 创建仓库（远程仓库会进入 `downloading` 状态并后台自动 clone） |
| POST | /api/git-repos/detect-local | 识别本地 Git 仓库信息（传入 path） |
| PUT | /api/git-repos/:id | 更新仓库 |
| DELETE | /api/git-repos/:id | 删除仓库（支持 body: `delete_local=true` 同时删除本地目录） |
...
### 文件筛选模式 `/api/file-filters`

| 方法 | 路径 | 说明 |
|------|------|------|
| GET | /api/file-filters | 获取列表（支持分页，内置预设排在前面） |
| GET | /api/file-filters/:id | 获取详情 |
| POST | /api/file-filters | 创建筛选模式（需传 name、include_extensions） |
| PUT | /api/file-filters/:id | 更新筛选模式 |
| DELETE | /api/file-filters/:id | 删除筛选模式（内置预设不可删除） |
...
| GET | /api/review-tasks/:id/issues | 获取任务发现的问题列表（支持 `?severity=&category=` 筛选） |
| POST | /api/review-tasks | 创建任务（仅允许仓库状态为 `ready` 且本地路径可用） |
| POST | /api/review-tasks/:id/execute | 提交任务执行（状态置为 pending；提交前会校验仓库状态） |
| PUT | /api/review-tasks/:id | 更新任务 |
| DELETE | /api/review-tasks/:id | 删除任务 |

## 仓库状态与评审前校验

- 默认下载目录：`data/repos`（相对项目根目录）
- 仓库状态：
  - `downloading`：远程仓库后台 clone 中
  - `ready`：仓库可用，可创建/触发/执行评审任务
  - `error`：仓库异常（如 clone 失败、目录不可用）
- 评审前校验入口：
  - 创建评审任务 `/api/review-tasks`
  - 手动执行任务 `/api/review-tasks/:id/execute`
  - 触发评审计划 `/api/review-plans/:id/trigger`
- 执行器在运行任务前也会再次检查仓库状态和本地 Git 路径，避免下载中或异常仓库进入评审流程
...
| git_repo | Git 仓库配置 |
| llm_config | LLM 模型配置 |
| file_filter | 文件筛选模式（项目类型预设，含包含扩展名和排除目录模式） |
| review_plan | 评审计划（定时触发等，可关联 file_filter） |
| review_task | 评审任务实例（可关联 file_filter） |
| review_log | 任务执行文本日志 |
| review_progress | 任务执行结构化进展（事件追加模式，支持五级粒度可视化） |
| review_issues | 评审发现的问题（按任务拆分，支持按严重程度/类别筛选） |
