...
- **LLM 配置管理** — 管理大语言模型配置（供应商、API Key、模型参数等），支持在线获取可用模型列表
- **评审计划管理** — 配置评审计划（名称、仓库、LLM、目标分支、触发时机），支持按间隔/每天定时自动触发，支持手动立即触发
- **评审任务管理** — 创建评审任务，关联仓库与 LLM 配置，查看执行结果；评审计划触发时自动生成
- **任务执行器** — 独立进程轮询数据库，串行执行待处理的评审任务
...
| 执行器 | @zouma/executor（独立进程，轮询执行评审任务） |
